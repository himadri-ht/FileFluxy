...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            f"{str(Path.home())}/.Fluxy/logs/server_{datetime.now().strftime('%d-%m-%Y_%H-%M-%S')}.log"
        ),
...
# Load share database
fluxy_db = TinyDB(f"{str(Path.home())}/.Fluxy/db/db.json")
            ...
            match request["type"]:
                ...
                case HeaderCode.UPDATE_HASH:
                    ...
                    if username is not None:
                        ...
                        try:
                            ...
                            user_share = fluxy_db.search(User.uname == username)[0]["share"]
                            ...
                            fluxy_db.update({"share": user_share}, User.uname == username)
                        except IndexError:
                            raise RequestException(
                                msg=f"Username does not exist",
                                code=ExceptionCode.NOT_FOUND,
                ...
                case HeaderCode.FILE_BROWSE:
                    ...
                    if username is not None and user_exists:
                        ...
                        browse_files: list[DBData] = fluxy_db.search(User.uname == request["query"].decode(FMT))
